...
def image_show(image, name):
    ...
    # image = cv2.resize(image, (1280, 1070), interpolation=cv2.INTER_AREA)
    show_image = cv2.resize(image, (320, 240), interpolation=cv2.INTER_AREA)
    name = str(name)
    ...
    # 拼接文件名
    global icount
    csv_filename = f'black-silver_{formatted_time}'
    count_0 = str(icount)
    path = os.path.join("./photo/", csv_filename + "_" + ms + ls + "_" + count_0 + ".bmp")
    print(csv_filename)
    icount = icount + 1

    # csv_filename = f'type_{formatted_time}'
    # path = os.path.join("./photo/", csv_filename+"_"+ms+ls+".bmp")
    # print(csv_filename)
    # import pdb;
    # pdb.set_trace()
    cv2.imwrite(path, image)
    cv2.imwrite("./000.bmp", image)
