...
def image_show(image, name):
    ...
    count_1 = str(icount_1)
    path = os.path.join("./photo_1/", csv_filename+"_"+ms+ls+"_"+count_1+".bmp")
    print(csv_filename)
    icount_1 = icount_1 + 1

    # font = cv2.FONT_HERSHEY_SIMPLEX
    # txt1 = f"File: {icount}"
    # cv2.putText(image, txt1, (150, 300), font, 2, (0, 200, 0), 6)



    cv2.imwrite(path, image)
    # cv2.imwrite("./111.bmp", image)
