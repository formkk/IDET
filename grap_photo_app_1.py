...
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QPushButton, QListWidget, QLabel, QWidget, QMessageBox, QStatusBar
)
...
class CameraApp(QMainWindow):
    def __init__(self):
        ...
        self.save_counter = 0  # 用于生成唯一的保存文件名
    ...
    def init_ui(self):
        ...
        self.stop_btn.setMinimumHeight(40)  # 增大按钮高度
    ...
    def save_image(self, image):
        ...
        if image is not None:
            # 生成唯一的文件名
            self.save_counter += 1
            file_name = f"captured_image_{self.save_counter}.png"
            # 保存图像
            cv2.imwrite(file_name, image)
            # 在状态栏显示保存的文件名
            self.status_bar.showMessage(f"已保存: {file_name}")
